...
public class DirectGraph 
{
	 ...
	 */
	public DirectGraph(String name) 
	{
	...
	public Vertex addVertex(String vertexName, int sdn) 
	{
		if (!m_vertices.containsKey(vertexName))
		{
			Vertex v = new Vertex(vertexName);
	...
	public int dinicsMaxFlow(Vertex source, Vertex target) 
	{
		...
		while (isAugmentingPathPresent(source, target))
		{
			...
			flow += df;
			System.out.println("Path #" + flow + " found");
		}
	...
	private void expandVertices(String source, String target)
	{
		...
		while (it.hasNext()) 
		{
			...
			if(vertexName != source && vertexName != target && !vertexName.contains("."))
			{
				...
				if(vertexName.startsWith("sdn"))
					{
						Edge e = getVertex(vertexName + ExpendSuffix_1).findEdge(getVertex(vertexName + ExpendSuffix_2));
						e.setCapacity(getVertex(vertexName).getMinCat());
					}
	...
	public DirectGraph reduction(String source, String target)
	{
		DirectGraph reductionGraph = new DirectGraph("rerduction");
		int sdns[] = {0,0,0,0};
		sdns[0]=0;
		sdns[1]=0;
		sdns[2]=0;
		...
		while (it.hasNext()) 
		{
			...
			if(sdns[a] == 0 && a != 0)
			{
				...
				reductionGraph.addVertex(ss);
			}
			
		}
		
		Iterator<Entry<String, Vertex>> it2 = m_vertices.entrySet().iterator();
		while (it2.hasNext()) 
		{
			Map.Entry<String, Vertex> entry =it2.next();
			vertexName = (String) entry.getKey();
			int a = entry.getValue().getSdn();
			...
			if(a == 0)
			{
				...
				reductionGraph.addVertex(vertexName);
				List<Edge> e = entry.getValue().getAllEdges();
				for(Edge tmp : e)
				{
					if(tmp.getFrom().getSdn()!=0)
					{
						String s = "sdn" + tmp.getFrom().getSdn();
						Vertex temp = getVertex(s);
						//temp.incMinCut();
						reductionGraph.getVertex(s).incMinCut();
						reductionGraph.addEdge(s, vertexName, 0, (short)0, (short)0);
						reductionGraph.addEdge(vertexName, s, 0, (short)0, (short)0);
	...
	public String shortToSting()
	{
		...
		for (Vertex v : m_vertices.values())
		{
			...
			for (Edge e : m_edges)
			{
				s+= "Edge: From:" + e.getFrom() + " To: " + e.getTo() + "\n"; 
